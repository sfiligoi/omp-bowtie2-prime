...
// This is the only place where we define the semantics of "enc"
// So keep it simple
// SImply 2 bit encoding, going backwards
static void encSeedToStr(uint64_t enc, uint8_t len, char *buf) {
...
static void multiseedSearchWorker() {
	...
	MyTimer tmr; 
	{
		...
		MultiSeedAlignerSimple *pals = new MultiSeedAlignerSimple(multiseed_ebwtFw);
		MultiSeedAlignerSimple& als = *pals;

		char *seedBuf = new char[num_parallel_tasks*multiseedLen*nthreads_multiplier];
		...
		for (uint64_t gloopi=0; gloopi<1000000; gloopi++) {  // TODO: Quick hack
		   ...
		   tmr.start();
		   als.clearSearchSeeds();

#ifdef USE_CUSTOM_ALLOCS
		   mate_allocs.ensure_spare();
#endif
		   als.reserveBuffersFixed(batch_parallel_tasks*nthreads_multiplier);
		   {
			   {
				char tstr[32];
				//encSeedToStr(currSeedEnc, multiseedLen, tstr);
				tstr[0] = 0;
				//fprintf(stderr, "Start currSeedEnc: %lu '%s'\n",currSeedEnc,tstr);
			   }
#pragma omp parallel for default(shared)
			for (uint32_t nb=0; nb<batch_parallel_tasks; nb++) {
			  uint64_t loopSeedEnc = currSeedEnc+ nb*nthreads_multiplier;
			  uint32_t soff = nb*nthreads_multiplier*multiseedLen;
			  for (uint16_t ib=0; ib<nthreads_multiplier; ib++) {
				encSeedToStr(loopSeedEnc, multiseedLen, seedBuf+soff);
				loopSeedEnc++;
				soff+=multiseedLen;
			  }
			}
...

		   {
			for (uint32_t nb=0; nb<batch_parallel_tasks; nb++) {
			  uint32_t soff = nb*nthreads_multiplier*multiseedLen;
			  for (uint16_t ib=0; ib<nthreads_multiplier; ib++) {
				  als.addSearchSeed(seedBuf+soff, multiseedLen);
				  soff+=multiseedLen;
			  }
			}
		   }
		...
		{
			for (uint32_t nb=0; nb<batch_parallel_tasks; nb++) {
			  uint64_t loopi = nb*nthreads_multiplier;
			  for (uint16_t ib=0; ib<nthreads_multiplier; ib++) {
				auto cnt = als.getSearchSeedSize(loopi);
				if (cnt<0) {
					logCnts[0]++;
				} else {
					bool found = false;
		...
		tmr.print();
		{
		  ...
		  for (int cnt=0; cnt<20; cnt++) {
			...
			if (cnt==0) {
				...
				for (int i=1; i<cnt; i++) ncnt*=2;
			}
			fprintf(stderr, "[%2i = %8i] %lu\n",cnt,ncnt,logCnts[cnt]);
			total += logCnts[cnt];
		  }
 ...
 * function.
 */
int bowtie_count_main(int argc, const char **argv) {
	try {
#ifdef WITH_AFFINITY
